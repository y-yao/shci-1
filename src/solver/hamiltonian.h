...
void Hamiltonian<S>::update_existing_elems(const S& system) {
  ...
  if (system.time_sym) {
    auto pf = static_cast<double(BaseSystem::*)(const size_t, const size_t, const int) const>(&BaseSystem::get_hamiltonian_elem_time_sym);
    matrix.update_existing_elems(std::bind(pf, &system, _1, _2, _3));
  } else {
    auto pf = static_cast<double(BaseSystem::*)(const size_t, const size_t, const int) const>(&BaseSystem::get_hamiltonian_elem);
    matrix.update_existing_elems(std::bind(pf, &system, _1, _2, _3));
  }
}
