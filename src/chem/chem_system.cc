...
double ChemSystem::find_connected_dets(
  ...
  if (second_rejection) {
    // Find approximate energy difference from HF det.
    // Note: Only using 1b energies as a proxy for det energies
  ...
  double thresh_fac = Config::get<double>("thresh_fac", 0.2);
  ...
  if (eps_min <= max_singles_queue_elem) {
	...
	if (second_rejection) {
	  ...
	  if (denominator > 0. && S * S / denominator < thresh_fac * eps_min * eps_min) {
  ...
  for (unsigned p_id = 0; p_id < n_elecs; p_id++) {
	...
	if (second_rejection) {
	  ...
	  if (denominator > 0. && H * H / denominator < thresh_fac * eps_min * eps_min) {
